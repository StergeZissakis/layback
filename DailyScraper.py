...
import pprint
import logging
from Utils import initialise_logger, sleep_for_seconds
...
def scrapeFootballSuperTips(db):
    ...
    for url in urls:
        ...
        for m in matches:
            ...
            if tommorow and int(date_time.hour) >= 6:
                continue;
...
def scrapeOrbitxch(db):
    ...
    while browser.wait_for_element_to_appear('//*[@id="biab_body"]/div[2]/main/div/div[3]/div/div/div[1]/div[3]/div/div[3]/div[1]', 1) == None:
    ...
    for tm in tommorowsMatches:
        ...
        try:
            ...
            logging.error("Failed to extract tommorow's start time: %s" % mTime )
            continue

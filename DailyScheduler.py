...
if __name__ == "__main__":
    ...
    matches = db.select("SELECT * FROM " + dummy.table_name + " WHERE url is not Null and date_time between (now() - interval '120 minutes') AND now() ORDER BY date_time ASC, id ASC;")
    for m in matches:
        mid = m[0]
        dateTime = m[3]
